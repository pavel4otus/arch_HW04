...
public class CatalogController {
    ...
    /*@Autowired
    public CatalogController(GoodsService goodsService, BrandService brandService, CategoryService categoryService, UserService userService){
    ...
    @GetMapping( value = "/microservices/v2/catalog", produces = MediaType.APPLICATION_JSON_VALUE) // + ";charset=UTF-8")
    ...
    public List<GoodDto> listGoodsByCategory(Principal principal, @RequestParam Long categoryId){
        ...
        list = goodsService.findByCategory(user, categoryId);
        return  list;
    }
    ...
    public List<GoodDto> listGoodsByBrand(Principal principal, @RequestParam Long brandId){
        User user = findUser( principal);
        List <GoodDto> list = new ArrayList<>();
        list = goodsService.findByBrand(user, brandId);
        return  list;
    }
